...
public class CaffeineListActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Hook up our support map fragment
    ...
    public void onMapReady(GoogleMap googleMap) {
        //Use latitude and longitude of each location to create markers on the map
        mMap = googleMap;
        //Loop through each location
        for (Location location : allLocationsList)
        {
            LatLng coordinate = new LatLng(location.getLatitude(), location.getLongitude());
            //Add marker for that coordinate
            mMap.addMarker(new MarkerOptions().position(coordinate).title(location.getName()));
        }
        // Change camera view to the marker
        LatLng currentPosition = new LatLng(33.671028, -117.971305);
        CameraPosition cameraPosition =
                new CameraPosition.Builder().target(currentPosition).zoom(14.0f).build();
